...
export const JsExercisePage = () => {

    var arr = []
    var mark = 0  

    const[form, setForm] = useState([])
    const JSQuestionId = "60ae77a8261cf4501c670a72"
    ...
    const fetchQuestions = useCallback( async () => {
        try{
            const fetched = await request(`/api/question/${JSQuestionId}`, 'GET', null)
            setQuestions(fetched)
        }catch(e) {}
    },[JSQuestionId, request]) //получение полных данных всех вопросов

    const fetchAnswers = useCallback( async () => {
        try{
            const fetched = await request(`/api/answer/${JSQuestionId}`, 'GET', null)
            setAnswers(fetched)
        }catch(e) {}
    },[JSQuestionId, request]) //получение полных данных всех ответов

    const SaveAnswers = async () => {
        try{
            await request('/api/useranswer/generate', 'POST', {mark, SubjectId: JSQuestionId}, {
