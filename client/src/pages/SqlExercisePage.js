...
export const SqlExercisePage = () => {

    var arr = []
    var mark = 0  

    const[form, setForm] = useState([])
    const SQLQuestionId = "60c7a25e8d35b136948a2c42"
    ...
    const fetchQuestions = useCallback( async () => {
        try{
            const fetched = await request(`/api/question/${SQLQuestionId}`, 'GET', null)
            setQuestions(fetched)
        }catch(e) {}
    },[SQLQuestionId, request]) //получение полных данных всех вопросов

    const fetchAnswers = useCallback( async () => {
        try{
            const fetched = await request(`/api/answer/${SQLQuestionId}`, 'GET', null)
            setAnswers(fetched)
        }catch(e) {}
    },[SQLQuestionId, request]) //получение полных данных всех ответов

    const SaveAnswers = async () => {
        try{
            await request('/api/useranswer/generate', 'POST', {mark, SubjectId: SQLQuestionId}, {
